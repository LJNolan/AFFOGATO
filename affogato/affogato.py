...
def make_galfitm_input(img_size, zp, img_scale, sky, comps, outputdir=".",
   ...
   - The reading in of "comps" is really only designed for PSFs and
     sersics - other components are TBD
...
def rp_plot(fig, gs, locators, comps=['unknown'], inputdir='.', outputdir='.',
   ...
   ylim : float, optional
      ...
      regions of the model from plotting. The default is 30., but setting to 0
...
def get_psf(fits_filepath, psf_dir, coord):
   ...
   an excellent fit to given data regardless, so the variation across the
   chip - already not easy to account for in drizzled images - is negligible
